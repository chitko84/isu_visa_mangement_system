...
/**
 * Notification count - query from notifications table
 * We'll try to query, if table doesn't exist yet, we'll catch the error
 */
...
try {
    ...
    if ($table_check && $table_check->num_rows > 0) {
        // Table exists, query for unread notifications
        $notification_query = "SELECT COUNT(*) as count FROM notifications WHERE student_id = ? AND is_read = 0";
        $stmt = $conn->prepare($notification_query);
        if ($stmt) {
            $stmt->bind_param("i", $student_id);
            $stmt->execute();
            $notification_result = $stmt->get_result()->fetch_assoc();
            $stmt->close();
            
            if ($notification_result) {
                $notification_count = (int)$notification_result['count'];
            }
        }
    }
} catch (Exception $e) {
    // If there's an error (table doesn't exist), notification_count remains 0
...
if ($profilePhoto && preg_match('/^https?:\/\//i', $profilePhoto)) {
    ...
    // Stored relative path (e.g. uploads/xxx.png)
    $profilePhotoUrl = ltrim($profilePhoto, '/');
} else {
